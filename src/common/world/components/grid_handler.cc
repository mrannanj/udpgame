...
void GridHandler::raycast_range(const vec3& p, const vec3& d,
				int ind[3][2]) const
{
	...
	for (unsigned a = 0; a < 3; ++a) {
		...
		ind[a][1] = (int)min((float)GRID_SIZE[a] - 1.0f,
				     max(p[a], k[a]));
	}
}
	...
	for (int x = ind[0][0]; x <= ind[0][1]; ++x) {
		for (int y = ind[1][0]; y <= ind[1][1]; ++y) {
			for (int z = ind[2][0]; z <= ind[2][1]; ++z) {
				...
				if (block and AABBvsRay(box, s, d, t, tmp_axis,
							tmp_dir)) {
					if (distance > t) {
						distance = t;
						hit = true;
