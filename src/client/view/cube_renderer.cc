...
CubeRenderer::CubeRenderer(const ResourceLocator& rl):
	Shader(rl, "resources/shaders/cube.vert",
...
void CubeRenderer::On() const
{
	...
	GLint position =
		glGetAttribLocation(shader_program, "position_modelspace_");
	...
	GLint normal =
		glGetAttribLocation(shader_program, "normal_modelspace_");
			    ...
			    const glm::mat4 & p) const
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 36 * 11, cube_vertices,
			GL_STREAM_DRAW);
