...
TextField::TextField(float left_x, float top_y, float right_x,
		     float font_size, const std::string& text):
	m_left_x(left_x),
	m_top_y(top_y),
	m_right_x(right_x),
...
void TextField::Draw(const Renderer& r) const
{
	...
	r.text_renderer.On();
	r.text_renderer.DrawText(m_left_x, m_top_y, m_font_size,
				 m_text.substr(0, (size_t) m_text_length),
				 m_bg_color);
}
